...
function pushNotif(type, title, msg) {
  ...
  var icons = {success:'fa-check-circle',info:'fa-info-circle',warn:'fa-exclamation-triangle',error:'fa-times-circle'};
  var colors = {success:'var(--ng)',info:'var(--nc)',warn:'#ffaa00',error:'#ff3355'};
  ...
  setTimeout(function(){t.classList.remove('show');t.classList.add('hide');setTimeout(function(){t.remove()},400)},4500);
}
  ...
  pollInterval = setInterval(function() {
    fetch('/api/status').then(function(r){return r.json();}).then(function(d){
      ...
      if(d.devices===0) {
        ...
        var html = '';
        for(var i=0;i<d.device_list.length;i++) {
          var dev = d.device_list[i];
          ...
          html += '<div style="flex:1"><div class="font-term" style="font-size:0.8rem;color:#ddd;">'+dev.name+'</div><div class="font-term" style="font-size:0.6rem;color:#666;">'+dev.ip+'</div></div>';
...
function sendCommand() {
  ...
  .then(function(r){return r.json();}).then(function(d){
    ...
    input.value = '';
  }).catch(function(){pushNotif('error','Error','Failed to send command');});
}

// ─── SEND ACTION ───
function sendAction(action) {
  var labels = {vibrate:'Vibration',flash:'Flash',notify:'Notification',alert:'Alert'};
  ...
  }).catch(function(){pushNotif('error','Error','Failed to send action');});
}
...
function sendMessage() {
  ...
  }).catch(function(){pushNotif('error','Error','Failed to send message');});
}

// ─── COPY LINK ───
function copyLink() {
  var link = document.getElementById('conn-link').textContent;
...
<div class="glass-card">
  <div style="margin-bottom:16px;">
    <i class="fas fa-shield-haltered" style="font-size:3rem;color:#00ff41;opacity:0.5;"></i>
  </div>
  <div class="font-cyber neon-green" style="font-size:1.2rem;margin-bottom:4px;">● CYBERTERMUX</div>
  <div class="font-term" style="connected_devices[cid]['pending_command'] = cmd
...
def api_client_register():
    ...
    log_entry = {
        ...
        'type': 'success',
        'label': 'CONNECT',
        'msg': f'Device connected: {data.get("name", "Unknown")} ({ip})'
    }
    session_logs.append(log_entry)
...
def api_client_disconnect(client_id):
    if client_id in connected_devices:
        ...
        log_entry = {
            ...
            'type': 'warn',
            'label': 'DISCONNECT',
            'msg': f'Device disconnected: {name} ({ip})'
        }
        session_logs.append(log_entry)
...
if __name__ == '__main__':
    ...
    print("║         v3.1 - Local Network Mode        ║")
    print("╠══════════════════════════════════════════╣")
    print(f"║  IP:  {host_ip:<32}║")
    print(f"║  Port:  5000                             ║")
    print("╠══════════════════════════════════════════╣")
    print("║  CONTROL PANEL:                          ║")
    ...
    print(f"║  http://{host_ip}:5000/client             ║")
    print("╠══════════════════════════════════════════╣")
